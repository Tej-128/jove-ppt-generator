...
1. Start every lesson with a definition/concept-setting slide before examples.
   - Example: If the lesson is "The Scientific Method", slide 1 must explain what the scientific method is before discussing cabbage/slugs/garlic examples.
   - If the lesson is "Correlation and Causation", first define correlation and causation, then show types/examples.
2. Never jump directly into examples before definitions.
3. When a slide uses a table for key terms, types, stages, or comparisons, the table must normally include:
   - Term/Step/Type
   - Definition/Meaning
   - Example/Application
...
6. If content has types, conditions, stages, or comparisons (2 or more), you MUST generate a 'table' slide with a Definition/Meaning column and Example/Application column where applicable.
...
def _normalize_slide(slide: dict, lesson_name: str, transcript: str, is_first_content_slide: bool = False) -> dict:
    ...
    if stype == "table":
        ...
        if len(headers) < 3 or not has_definition or not has_example:
            # Do not destroy original rows; this nudges common table format safely.
            slide["headers"] = ["Term/Step/Type", "Definition/Meaning", "Example/Application"]
            new_rows = []
...
def _validate_slide_payload(payload: dict, lesson_name: str, transcript: str) -> dict:
...
- Slide 2/table subtitles must be specific and suitable to the table.
...
- glossary_terms: comprehensive for THIS lesson's transcript content."""
    ...
    return _validate_slide_payload(payload, lesson_name, transcript)
